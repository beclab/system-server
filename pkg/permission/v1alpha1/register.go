...
func ValidateAppKeyWithRequest(appKey string, op string, req *restful.Request) error {
	datatype := req.PathParameter(api.ParamDataType)
	version := req.PathParameter(api.ParamVersion)
	group := req.PathParameter(api.ParamGroup)

	signature := req.HeaderParameter("X-Auth-Signature")
	err := ValidateAppKey(appKey, op, datatype, version, group, signature)
	if err != nil {
...
func ValidateAppKey(appKey, op, dataType, version, group, signature string) error {
	...
	aps, err := sysClient.SysV1alpha1().ApplicationPermissions("").List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return err
	}
